...
tags: ["query:sr"]
score: 8.0
evidence: 从噪声和损坏的数据中恢复物理规律和控制方程
tldr: 针对物理信息神经网络（PINN）在处理复杂噪声和异常值时性能下降的问题，本文提出了噪声自适应物理信息神经网络（naPINN）。该方法通过嵌入基于能量的模型（EBM）来学习预测残差的潜在分布，并利用可训练的可靠性门自适应过滤高能量数据点。实验证明，naPINN在多种偏微分方程基准测试中表现优异，能够有效隔离异常值并准确重建物理动力学，具有极强的鲁棒性。
selection_source: fresh_fetch
motivation: 传统的PINN在面对非高斯噪声和严重异常值时鲁棒性不足，且通常需要预先了解噪声分布的先验知识。
method: 通过嵌入基于能量的模型学习残差分布，并利用可训练的可靠性门和拒绝成本正则化自适应地过滤异常数据。
result: 在多种受非高斯噪声和高比例异常值干扰的PDE基准测试中，naPINN的性能显著优于现有的鲁棒PINN基准方法。
conclusion: naPINN提供了一个无需先验噪声知识即可从严重损坏的测量数据中稳健恢复物理规律的有效框架。
---

## 摘要
物理信息神经网络 (PINNs) 是解决反问题和从观测数据中发现控制方程的有效方法。然而，在复杂的测量噪声和严重离群值的影响下，其性能会显著下降。为了解决这一问题，我们提出了噪声自适应物理信息神经网络 (naPINN)，它能够在无需噪声分布先验知识的情况下，从受损测量中稳健地恢复物理学解。naPINN 在训练循环中嵌入了一个基于能量的模型，以学习预测残差的潜在分布。利用学习到的能量景观，一个可训练的可靠性门控自适应地过滤掉表现出高能量的数据点，同时拒绝成本正则化防止了丢弃有效数据的平凡解。我们在受非高斯噪声和不同比例离群值干扰的各种基准偏微分方程上证明了 naPINN 的有效性。结果表明，naPINN 显著优于现有的稳健 PINN 基准模型，成功隔离了离群值，并在严重数据损坏的情况下准确重建了动力学过程。
