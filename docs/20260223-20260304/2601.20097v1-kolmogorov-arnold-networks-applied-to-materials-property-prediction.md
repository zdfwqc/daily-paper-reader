...
tags: ["query:sr"]
score: 7.0
evidence: 将作为可解释回归模型的KAN与其他方法进行比较
...
## 摘要
Kolmogorov-Arnold 网络 (KANs) 被提出作为基于多层感知器 (MLP-NNs) 的传统神经网络架构的替代方案。KANs 相对于 MLP-NNs 的潜在优势，包括显著增强的参数效率和更高的可解释性，使其成为监督机器学习问题中一种极具前景的新型回归模型。我们将 KANs 应用于材料性能预测，重点关注由实验和计算数据组成的 33 种不同性能的集合。我们将 KAN 的结果与随机森林进行了比较，随机森林是一种通常在极少优化的情况下就能在广泛的性能预测中表现出色的方法。KANs 的表现分别在约 35%、60% 和 5% 的情况下差于、持平或优于随机森林，且在实践中 KANs 比随机森林更难拟合。通过调整网络架构，我们发现性能拟合的误差通常比标准 KAN 低 10-20%，且通常能得到与随机森林相当的结果。在预测反应堆压力容器转变温度偏移的具体背景下，我们通过比较简单 KAN 模型（例如，参数少于 50 个）及其拟合建议的闭式表达式与先前发表的深度 MLP-NNs 以及利用脆化物理领域专业知识创建的手动调整模型，探索了 KANs 的参数效率和可解释能力。我们发现，简单的 KAN 模型及其产生的闭式表达式所产生的预测误差与具有相当参数数量的成熟手动调整模型相当，且在生成过程中基本上不需要领域专业知识。这些发现增强了 KANs 在材料科学机器学习中的潜在适用性，并表明应将 KANs 作为预测材料性能的回归模型进行探索。
